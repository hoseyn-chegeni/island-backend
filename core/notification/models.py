...
class Otp(models.Model):
    otp_status = models.CharField(
        max_length=20, choices=OtpStatus.choices, default=OtpStatus.IN_PROGRESS
    )
    otp_type = models.CharField(
        max_length=10, choices=OtpTypes.choices, default=OtpTypes.EMAIL
    )
    otp_function = models.CharField(
        max_length=20,
        choices=OtpFunction.choices,
        default=OtpFunction.REGISTER,
        null=True,
        blank=True,
    )
...
class ReserveRequest(models.Model):
    ...
    status = models.CharField(
        max_length=20,
        choices=ReserveRequestStatus.choices,
        default=ReserveRequestStatus.IN_PROGRESS,
    )
